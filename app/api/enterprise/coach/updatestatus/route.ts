...
const coachUpdate=await db.update(coaches).set({ status:'Active'}).where(eq(coaches.id, parseInt(coach_id))).execute();
...
const updateResult = await db
    ...
    if(coach.email)
    {
      const emailResult = await sendEmail({
        to: coach.email,
        subject: "D1 NOTES Coach Account Update",
        text: "D1 NOTES Coach Account Update",
